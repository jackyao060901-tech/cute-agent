# SOXX Sector Smart Money Scan — 独立复核包 (Review Packet) v2

> 目的:让外部大模型**不依赖我方代码**即可独立核验全部数字与口径。
> 全部由真实 fixture 程序化生成。数据源:LLMQuant Data(SEC N-PORT / 13F / 价格)。
> v2 变更:据上轮复核反馈,改进 C4/C5 表达、补 D 段对照表、E 段精确化并标注假设。
...
### B4. SOXX 价格窗口
- 请求窗口 2026-03-23~2026-06-17(文章称『90 天』;**实为约 3 个月 / 86 自然日 / 61 交易日**)
...
- **C5 完整对账(显式列全 5 项,避免误读为只加股票)**:
  ```
  Top5 + 其他前十大 + 其余equity + cash + excluded
  = 35.5728% + 20.7725% + 43.5316% + 2.1352% + (-0.0004%)
  = 102.0116%   (= 总权重 102.0116% ✓)
  ```
  其中『仅股票部分』= Top5+其他前十+其余equity = 99.8768%(**不是总权重**)。
...
> 注:此处『差异』是**与文章公布值之差**;并不预设文章为基准真值——两者估值基准可能不同。
| ticker | 我方 | 文章 | 我方相对文章 |
|---|---|---|---|
...
- **假设(非事实)**:文章更高值**可能**源于不同估值基准(如更晚的市价重估)。此为 *hypothesis*,我方未掌握文章的计算细节,故不作为结论。
...
2. C2–C7 是否能用 B 段原始数据手工重算复现;特别核对 C5 五项相加 = 总权重。
3. D1/D2 对账是否成立。
4. E1 差异范围与 E2 的『处理/证据/假设』三分是否专业、是否还有更稳妥做法。
